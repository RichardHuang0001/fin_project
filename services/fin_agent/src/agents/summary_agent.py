...
def load_finr1_model(model_path="/root/code/Finance/FinR1"):
    ...
    if torch is None or AutoTokenizer is None or AutoModelForCausalLM is None:
        raise RuntimeError("Local model dependencies are not installed. Set USE_LOCAL_MODEL=api or install torch/transformers.")
...
def generate_report_with_finr1(model, tokenizer, prompt, max_new_tokens=1200):
    """使用FinR1模型生成报告"""
...
async def summary_agent(state: AgentState) -> Dict[str, Any]:
    ...
    def clip_text(text, max_chars=1200):
        if not isinstance(text, str):
            text = str(text)
        return text if len(text) <= max_chars else text[:max_chars] + "\n...(truncated)"

    fundamental_analysis = clip_text(fundamental_analysis, 1200)
    technical_analysis   = clip_text(technical_analysis, 1200)
    value_analysis       = clip_text(value_analysis, 1200)
    news_analysis        = clip_text(news_analysis, 1200)
    ...
    try:
        ...
        if model_choice == "local":
            ...
            llm = ChatOpenAI(
                ...
                max_tokens=1200   # 增大输出长度以生成更详细的综合报告
            )
