...
class BaostockDataSource(FinancialDataSource):
    ...
    def _resolve_local_model_paths(self, adapter_dir_name: str) -> tuple[Optional[str], Optional[str]]:
        """解析本地模型目录；缺失时返回(None, None)。"""
        env_base_model = os.getenv("FINANCE_QWEN_BASE_MODEL")
        env_adapter_model = os.getenv(f"{adapter_dir_name.upper()}_DIR")

        project_root = Path(__file__).resolve().parents[2]
        default_base_model = project_root / "Qwen"
        default_adapter_model = project_root / adapter_dir_name

        base_model_path = Path(env_base_model) if env_base_model else default_base_model
        adapter_model_path = Path(env_adapter_model) if env_adapter_model else default_adapter_model

        if not base_model_path.exists():
            logger.info(f"未找到本地基础模型目录，跳过本地模型加载: {base_model_path}")
            return None, None
        if not adapter_model_path.exists():
            logger.info(f"未找到本地适配器目录，跳过本地模型加载: {adapter_model_path}")
            return None, None

        return str(base_model_path), str(adapter_model_path)
    ...
    def _load_risk_model(self):
        ...
        try:
            ...
            base_model_name, risk_model_path = self._resolve_local_model_paths("qwen_risk_model")
            if not base_model_name or not risk_model_path:
                return None, None
    ...
    def _load_sentiment_model(self):
        ...
        try:
            ...
            base_model_name, sentiment_model_path = self._resolve_local_model_paths("qwen_sentiment_model")
            if not base_model_name or not sentiment_model_path:
                return None, None
